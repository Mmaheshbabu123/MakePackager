...
namespace  Packages\MakePackager;
...
use Packages\MakePackager\Exceptions\InvalidAssetPath;
use Packages\MakePackager\Exceptions\ModuleNotFoundException;
use Packages\MakePackager\Json;
...
abstract class FileRepository implements RepositoryInterface, Countable
{
    ...
    public function Modulescan()
    {
        ...
        foreach ($paths as $key => $path) {
            ...
            foreach ($manifests as $manifest) {
                $name = Json::make($manifest)->get('name');
                $packageNameSpace = Json::make($manifest)->get('namespace');
                $modules[$index]['name'] = $name;
                $modules[$index]['namespace'] = $packageNameSpace;
                $index++;
            }
        }
       logger($modules);
     ...
     * @inheritDoc
     */
    public function findInModuleNameSpace(string $name)
    {
        foreach ($this->Modulescan() as $index => $module) {
